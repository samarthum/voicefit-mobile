...
- [ ] **M1** `dashboard.tsx:1988` — "+ Add Item" on meal review is non-functional. Styled as link, plain `<Text>`.
- [ ] **M2** `dashboard.tsx:2005-2011` — "Eaten at" row looks tappable (chevron) but isn't.
- [ ] **M3** `dashboard.tsx:2125-2131,375-376` — "Add to session" row non-interactive, label hardcoded to "Morning Push".
- [ ] **M4** `dashboard.tsx:3186-3207` — Confidence pill always green even for "Low confidence".
- [ ] **M5** `dashboard.tsx:1902-1909` — "Edit text" button re-interprets instead of letting user edit first.
- [ ] **M6** `dashboard.tsx:926` — `dayOptions` memo has `[]` deps — stale after midnight.
- [ ] **M7** `dashboard.tsx:751-758` — WeightSparkline is fully hardcoded SVG, doesn't reflect real data.
- [ ] **M8** `dashboard.tsx:802` — Trend goal line hardcoded to 2000 cal, ignores user's actual goal.
- [ ] **M9** `dashboard.tsx:1233-1247` — "question" intent auto-saves without showing the answer to the user.
- [ ] **M10** `dashboard.tsx:301` — `getMealVisualKind` classifies all "rice" meals as salmon icon.
- [ ] **M11** `dashboard.tsx:1926,2034` — Missing `keyboardDismissMode` on review ScrollViews.
- [ ] **M12** Multiple files — No `Keyboard.dismiss()` after saves across multiple screens.
- [ ] **M13** `workout-session/[id].tsx:353-359` — "Previous" column shows prior set in same session, not historical data from prior sessions.
- [ ] **M14** `workout-session/[id].tsx:767-768` — KG/Reps headers shown for cardio exercises. Should show Duration.
- [ ] **M15** `workout-session/[id].tsx:675` — `router.replace` for back navigation loses history.
- [ ] **M16** `workouts.tsx:218,243` — Toast timer not cleaned up on unmount.
- [ ] **M17** `workouts.tsx:221-228` — No pagination, only first 20 sessions shown.
- [ ] **M18** `meals.tsx:172-178` — Edit panel doesn't auto-close after save.
- [ ] **M19** `meals.tsx:148-155` — Stale error/success persists when switching meals.
- [ ] **M20** `settings.tsx:341` — Sign out has no confirmation dialog.
- [ ] **M21** `settings.tsx:310-339` — Settings rows look tappable (chevron) but aren't.
- [ ] **M22** `coach.tsx:372-379` — Coach mic button navigates away to dashboard instead of recording in-context.
- [ ] **M23** `app/_layout.tsx` — No StatusBar component. Text may be invisible in dark mode with white bg.
- [ ] **M24** `feed.tsx:47` vs `dashboard.tsx:164` — Two different "today" utilities (UTC vs local) produce different dates near midnight.
- [ ] **M25** All files — COLORS duplicated 11 times across files.
- [ ] **M26** `dashboard.tsx` + `feed.tsx` — `ensureQuickSession` duplicated.
- [ ] **M27** `components/MealGlyph.tsx` vs `dashboard.tsx` — Divergent MealGlyph implementations with different matching logic.
- [ ] **M28** `coach.tsx:258` — No safe area on coach composer bottom, overlaps home indicator.
- [ ] **M29** `sign-in.tsx:125,136` — OAuth buttons not disabled during submission. Can trigger both flows simultaneously.
